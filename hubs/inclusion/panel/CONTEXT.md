...
**Action Update**:
A dated, attributed entry on an Action's thread saying *what was tried* — "called the parent, no answer". A historical event, not a statement that stops being true: entries accumulate, read oldest-first, and the newest one describes where the action has got to in English. Status is unchanged by an update and stays three values; an update can still be added to a `complete` or `not_needed` action. No sensitivity of its own — an update is exactly as visible as the Action it hangs off. Built on the shared `core.ThreadEntry` abstract base, so a soft-deleted entry leaves the thread without leaving the table. Its author (and only its author) can edit it in place — the entry gains an "edited" marker beside its timestamp rather than a version history, since the point is that the reader can tell it wasn't what was written on the day — or soft-delete it; both are enforced against the current-staff cookie identity server-side, not just by which entries the template renders a button on (`inclusion_panel_action_new`'s `edit_action_update`/`delete_action_update` handling). The edit/delete buttons and inline edit form are one shared template partial (`_thread_entry_edit_delete.html`), not Action-specific markup, so the meeting-note thread gets the identical UI once it moves onto the same base (#236). See [docs/adr/0031-three-kinds-of-note-and-one-thread-entry-base.md](../../../docs/adr/0031-three-kinds-of-note-and-one-thread-entry-base.md) for the three-way note taxonomy.
