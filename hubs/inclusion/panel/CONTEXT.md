...
Marking an action Not Required is the one transition of the three that begs a reason and (before #235) recorded none, so every surface that can make that change (Actions list row, Home's My Actions card, Discussion's inline row, Referral Details' Actions section) offers a one-line, optional, skippable note at that moment only — text entered becomes an ordinary Action Update, indistinguishable from one posted any other way. `apply_action_status` (`views/shared.py`) is the one entry point all four surfaces route the status change (and optional note) through.
