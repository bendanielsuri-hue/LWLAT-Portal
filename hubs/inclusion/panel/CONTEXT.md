...
**PanelGroup**:
The standing group of staff/experts scoped to one `School` who sit on panels together — think "the Woodstock SEND panel" as an institution, independent of any single meeting date. Holds a `default_chair`. A `Panel` (meeting) belongs to one `PanelGroup`, or none (ungrouped). One `PanelGroup` is MAT-wide (`school=null`) rather than scoped to a school — reserved for **MAT Panel Meetings** (see below), not the same thing as a `Panel` left ungrouped.
_Avoid_: Panel (too easily confused with the meeting-instance model)

**PanelGroupMember**:
Standing membership in a `PanelGroup` — staff or an `ExternalContact`, optionally tagged with an `Expertise`. Distinct from `PanelMember`, which is per-meeting attendance. Coming off the roster is **deactivation**, never deletion, and it is a decision somebody made about somebody else: it records when, who did it (`deactivated_by`) and why (`deactivation_reason` — a **Preset Reason** or free text, see below), and the roster screen reads those three back as one line. Rejoining clears all three rather than accumulating history — a member who is back on the roster has no deactivation to describe.
_Avoid_: Member (ambiguous with PanelMember), Remove/Delete a member (the row and its history stay)
...
A referral's escalation state is binary: **at most one open Escalation per referral at a time**, though the model keeps full history (a referral can be escalated, resolved, and escalated again later — same history-preserving shape as `SafeguardingNote`'s superseding). The Escalate action must guard against creating a second open Escalation for a referral that already has one. A Referrals/Actions row shows this as its own independent pill (`_referral_escalation_pill`, same convention and call sites as `_referral_review_pill` — Referrals list and Actions list) reading simply "Escalated" whenever an open Escalation exists, never a count.

**MAT Panel Meeting**:
Not a separate model — a `Panel` belonging to the MAT-wide `PanelGroup`, distinguished from a school `Panel` purely by which group it's in. Chaired the same way as any Panel (`default_chair`/`chair_follows_default`), not by whoever triggers Quick Launch. The MAT-wide `PanelGroup` carries a real standing `PanelGroupMember` roster (staff frequently involved in MAT-level decisions) — the same convention as a school panel's default roster; letting a MAT meeting have ad-hoc invited/attending members beyond that roster is deliberately out of scope here, tracked separately in [#161](https://github.com/bendanielsuri-hue/LWLAT-Portal/issues/161).

Eligibility is strict: its agenda (`PanelReferral` rows) may only ever contain referrals with an open `Escalation`, MAT-wide rather than scoped to one school (unlike a school Panel's normal eligibility/`_due_followups`) — there is deliberately no path to add a non-escalated referral to a MAT agenda; if one needs MAT-level attention, it gets escalated first. This is the same generic "unassigned referrals" eligibility list `inclusion_panel_meeting_setup` builds for any Panel, just filtered differently for the MAT group.

**Only one MAT Panel Meeting may be `running` at a time** — a hard invariant enforced everywhere (Quick Launch, and Start Meeting on a separately-scheduled MAT panel), unlike school Panels, which have no such limit. Adding a newly-escalated referral to an already-running MAT meeting reuses the existing generic "Add to Agenda" mechanism (`meeting_agenda.html`'s link into `inclusion_panel_meeting_setup`'s `add_referral` action, which already works append-only against a `running` panel with no changes needed) — it is not a new composition capability, just a new caller of an existing one. A running MAT panel's Agenda page shows a page-load-computed count of open Escalations not yet on its agenda, prompting the chair to use that same link.

Usually reached via "Quick Launch" from the Escalations screen for one urgent, one-student case. Quick Launch's behaviour branches on whether a MAT panel is already `running`: if not, it creates one directly in `running` status (skipping `draft`/`ready`) with that one referral's `PanelReferral` pre-added and discussion already started; if one is already running, it appends the referral to that panel's agenda instead of creating a second one. A MAT panel can still be scheduled the normal way (create → agenda setup → start) when several escalations are known about ahead of time and worth discussing together in one sitting — subject to the same one-running-at-a-time gate.

**Preset Reason** (`PresetReason`):
One of the sentences offered in a dropdown wherever the panel asks *why* — escalating a referral to MAT level, and taking someone off a Panel Group's roster. Admin-managed on Panel Settings' Preset Reasons tab (added, and withdrawn by deactivation, the same way Action Categories are), and scoped by **context** so a preset only appears on the form it was written for. Both places offered a hardcoded list until [#239](https://github.com/bendanielsuri-hue/LWLAT-Portal/issues/239).
A preset is a *suggestion*, never a constraint: every form pairs the dropdown with an "Other" free-text box, and whichever the user picks, the consumer stores the resulting **sentence** as its own free text (`Escalation.reason`, `PanelGroupMember.deactivation_reason`). Nothing points back at the preset row, so rewording or withdrawing a preset changes only what the next person is offered — never what somebody already said. A new asking context is a new `CONTEXT_*` value on the one table, not a second table.
_Avoid_: Reason code, reason type (there is no coded value — the reason is the sentence), Reason Category
