...
- `_post_action_update(request, action)` (`views/actions.py`) — the one place an `ActionUpdate` actually gets created from a POST. Both `inclusion_panel_action_new`'s Updates step (inside an edit, referral in the URL) and `inclusion_panel_action_add_update` (action_id alone — My Actions' inline add, the Actions list row's Add Update button) call this rather than each creating their own row, so an update posted from a list is indistinguishable from one posted in the modal. The second view's own `get_object_or_404(visible_actions_for(...), ...)` is the sensitivity gate for that path; the first inherits its gate from the edit lookup above it.
