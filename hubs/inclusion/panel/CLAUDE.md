...
See [CONTEXT.md](CONTEXT.md) for the domain glossary (Referral vs PanelReferral, Panel vs PanelGroup, discussion stages, etc.). Portal-wide design/interaction rules live as principles at the repo root (`PRINCIPLES-DESIGN.md`/`PRINCIPLES-INTERACTION.md`, `DES-`/`INT-` prefixes); Panel's own visual patterns (status pill modifiers, meeting-card anatomy) are colocated as comments directly in `panel.css`. See "Key helpers" below and "Motion/interaction helpers" for which JS/CSS helper to reach for.
...
`views.py` was one 4,390-line module until #220 split it into `views/`, one module per area (Home, Students, Referrals, Actions, Escalations, Meetings, Agenda, Discussion, Safeguarding, Admin, plus `base`/`shared`/`meeting_shared` below them). `views/__init__.py`'s docstring is the map — read it rather than a copy here — and it re-exports every name, so `panel.views.X` still resolves from `urls.py` and anywhere else.

## Key helpers

The area module each lives in is named in brackets.

- `_is_panel_staff(staff)` (`views/shared.py`) — lightweight role check: `PanelGroupMember.objects.filter(staff=staff).exists()`. Controls sensitive `ActionCategory` visibility and `SafeguardingNote` read access (writing is gated further, to `core.models.Staff.is_dsl`). No real auth yet.
- `apply_action_status(action, status, note, author)` (`views/shared.py`) — single owner for changing an `Action.status` (`completed_at` bookkeeping included); every status-change endpoint (`inclusion_panel_action_set_status`, `inclusion_panel_action_inline_update`, `inclusion_panel_action_status_update`) routes through it rather than setting `status` directly, so the optional Not Required note (#235) can't be reintroduced only some of the places status changes. `note` is filed as an ordinary `ActionUpdate` when non-blank, regardless of which status was set - only the front end (`action-not-required-prompt.js`) restricts *when* a note box is ever offered.
...
Both were underscore-private functions inside the old single `views.py` until they were lifted out — see [ADR 0019](../../../docs/adr/0019-panel-reconciliation-is-one-entry-point-with-an-injected-clock.md). Tests live in `tests/test_lifecycle.py` and `tests/test_reconcile.py`.
...
- Cascading/dependent options (e.g. Reg Group scoped by Year Group): pass a `{parent_value: [child_options]}` JSON map (`reg_groups_by_year_json` in `hubs/inclusion/views.py::inclusion_hub`, same shape as `forms_by_year_json` in `hubs/inclusion/panel/views/students.py::inclusion_panel_students`) and rebuild the dependent `<select>`'s options on the parent's `change`. Call `selectEl._uiSelect.refresh()` afterward if the `<select>` has been enhanced by `enhanceFormControls()` — its popover caches options at enhance time.
...
## Motion/interaction helpers (`panel.js`)

Mechanics are commented inline at each function — this covers *when to reach for which helper*, not a restatement of the code.

- **Search**: the shared search-box behaviour (`INT-P4`) queries `inclusion_panel_search` (`views/search.py`, one view parameterized by `kind`) — see [docs/adr/0006-shared-search-endpoint-server-fetch-pickers.md](../../../docs/adr/0006-shared-search-endpoint-server-fetch-pickers.md) for why every DB-backed picker fetches server-side rather than filtering a pre-rendered list. Switching a picker's source (e.g. Add Member's staff/external segmented control) keeps whatever's typed and re-queries against the new source rather than clearing it — only the "Change" button on an already-selected result clears the search text. See `initMemberPicker`'s `showPicker(keepSearchText)`.
- **Fade toggle**: `window.setFadeHidden(el, hide)` — for a small, same-size element popping in/out of an already-open dialog without changing its height (e.g. Add Member's footer button swapping to Back to New External Contact). Not a substitute for a height-changing swap (below); the two combine (rare) by wrapping this inside the height-transition's mutate callback.
- **Modal content swap (height transition)**: `window.animateModalHeightChange(dialog, mutate)` is the general rule for any modal content change that alters the dialog's natural height — not just an explicit view/step swap, also a live filter narrowing an already-rendered list. Panel-specific "growable" modals (content resizes repeatedly while open, so anchored near the top rather than vertically centred — `top: 8vh`, see `dialog#panel-search-dialog[open]` in `panel.css`): Panel Search, Panel Group Edit, New Referral (its student search re-filters live even though the picker-vs-form step itself is a one-off switch). A dialog that only switches once between fixed modes (e.g. Panel Meeting/Settings' inline "Create Panel Meeting" ↔ "Create Panel Group" swap) still animates the height change but stays centred.
- **Row grow-in / shrink-fade-out**: helpers (`shrinkAndFadeOut`, `growIn`, `flash`, `cancelRowAnim`, `wireRowRemoveForm`) live as top-level functions in `panel.js`, above `initAgendaDragDrop` — reuse directly rather than reimplementing per page.
- **Refreshing a live fragment in place**: Panel Group modal's `render()` is the reference implementation for preserving scroll position of a scrollable descendant across an `innerHTML` refresh, and suppressing replay of one-shot entrance animations (`.tab-row-fade-in`) on a refresh (vs. first render).
- **Count-delta pulse**: `window.pulseCount(el, 'up'|'down')` adds/removes `.count-pulse-up`/`.count-pulse-down` on whichever element holds the number. A page with its own drag-and-drop/fragment-swap flow (Panel Agenda Setup, Panel Agenda) marks each countable element with `data-count-key`/`data-count`, and `initAgendaDragDrop`'s shared `applyFreshDoc` diffs old vs. new automatically on every column swap. A page recomputing counts purely from remaining DOM rows (e.g. Home's My Referrals after a delete) uses `window.recountTabsFromRows` instead. Applies (`INT-M2`): bump and pulse synchronously the moment the triggering click lands, never deferred to wait for a removal animation elsewhere. See `optimisticallyDecrementCounts` (drag-and-drop) and the `toggle_group_member_active` handler (Edit Panel Group) for the two current instances.

## Constants

All four live in `views/base.py`.
