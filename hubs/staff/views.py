from django.shortcuts import render

from core.hub_context import hub_context
from core.models import Staff

STAFF_MENU = [
    {'name': 'Staff Dashboard', 'url': '/staff/dashboard/', 'icon': 'staff/icons/staff_dashboard.svg', 'module_key': 'staff_dashboard'},
    {'name': 'Staff Reports', 'url': '/staff/reports/', 'icon': 'staff/icons/reports.svg', 'module_key': 'staff_reports'},
    {'name': 'My Timetable', 'url': '/staff/my-timetable/', 'icon': 'staff/icons/timetable_grid.svg', 'module_key': 'staff_my_timetable'},
    {'name': 'Staff Directory', 'url': '/staff/directory/', 'icon': 'staff/icons/directory.svg', 'module_key': 'staff_directory'},
    {'name': 'Absence Request', 'url': '/staff/absence-request/', 'icon': 'staff/icons/absence.svg', 'module_key': 'staff_absence_request'},
    {'name': 'Payslips', 'url': '/staff/payslips/', 'icon': 'staff/icons/payslips.svg', 'module_key': 'staff_payslips'},
    {'name': 'CPD & Training', 'url': '/staff/cpd-training/', 'icon': 'staff/icons/training_cap.svg', 'module_key': 'staff_cpd_training'},
    {'name': 'Staff Calendar', 'url': '/staff/calendar/', 'icon': 'staff/icons/calendar_plain.svg', 'module_key': 'staff_calendar'},
    {'name': 'Assessment Calendar', 'url': '/staff/assessment-calendar/', 'icon': 'staff/icons/calendar_assessment.svg', 'module_key': 'staff_assessment_calendar'},
    {'name': 'School Map', 'url': '/staff/school-map/', 'icon': 'img/icons/ui/map_pin.svg', 'module_key': 'staff_school_map'},
]


def _hub_context(request):
    return hub_context(request, STAFF_MENU, 'Staff')


def staff_hub(request):
    return render(request, 'hubs/staff/hub.html', _hub_context(request))


def staff_dashboard(request):
    return render(request, 'hubs/staff/dashboard.html', _hub_context(request))


def staff_reports(request):
    return render(request, 'hubs/staff/reports.html', _hub_context(request))


def staff_my_timetable(request):
    return render(request, 'hubs/staff/my_timetable.html', _hub_context(request))


def staff_directory(request):
    return render(request, 'hubs/staff/directory.html', {
        **_hub_context(request),
        'staff_list': Staff.objects.filter(is_active=True),
    })


def staff_absence_request(request):
    return render(request, 'hubs/staff/absence_request.html', _hub_context(request))


def staff_payslips(request):
    return render(request, 'hubs/staff/payslips.html', _hub_context(request))


def staff_cpd_training(request):
    return render(request, 'hubs/staff/cpd_training.html', _hub_context(request))


def staff_calendar(request):
    return render(request, 'hubs/staff/calendar.html', _hub_context(request))


def staff_assessment_calendar(request):
    return render(request, 'hubs/staff/assessment_calendar.html', _hub_context(request))


def staff_school_map(request):
    return render(request, 'hubs/staff/school_map.html', _hub_context(request))
