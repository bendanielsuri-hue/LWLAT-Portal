...
A menu entry gated by something other than the Module system (e.g. a role flag like `Staff.is_dsl`, for a page a whole class of staff should see regardless of the sidebar's module rollout state) skips `module_key`/`filter_by_module` entirely and is appended to `local_menu` by hand, conditionally, after the usual filtering — see `hubs/inclusion/panel/views.py`'s `_panel_base_context` (Safeguarding Briefings) for the pattern.
