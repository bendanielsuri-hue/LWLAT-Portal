...
`School`, `core.models.CategorySettings` (one row per `School.CATEGORY_CHOICES`), and the singleton `core.models.MatSettings` (forced `pk=1`) all share the same seven optional fields: `student_term`, `staff_term`, `portal_title`, `accent_colour`, `logo_url`, `support_email`, `support_phone`. Blank means "inherit from the next tier down." `core.portal_settings.resolve_portal_settings(request)` resolves each field independently: School → Category → MAT → hardcoded Python constant, keyed entirely off the currently-selected school (`core.identity.current_school_key`) — deliberately not off the viewer's own identity/home school. `portal.views.build_sections`/`build_hub_nav` additionally use `student_term`/`staff_term` to override the `Student`/`Staff` hub labels, taking priority over `Module.name` for those two entries. See [ADR 0003](../docs/adr/0003-tiered-portal-settings-resolution.md).

**Request cache**:
`core.request_cache.per_request(request, key, build)` — one value per HTTP request, stashed on the request object. Four things go through it, all of them asked for repeatedly by the seven portal context processors and again by the view: `module_map()`, `resolve_portal_settings()`, `current_staff()` and `portal.views.build_sections()`. Every one is derived either from a switcher cookie (set client-side, followed by a reload, so it can't change mid-request) or from rows only Portal Admin edits via POSTs that redirect rather than render — which is why the cache needs no invalidation. Deliberately per-request, never process-wide. `core/tests/test_request_cache.py` guards the invariant as query counts, since it's otherwise invisible to a test that only checks results. See [#193](https://github.com/bendanielsuri-hue/LWLAT-Portal/issues/193).
_Avoid_: Memoisation/cache (unqualified — both imply a lifetime longer than one request, which this explicitly isn't)
