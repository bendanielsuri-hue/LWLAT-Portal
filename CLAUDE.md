...
- **Portal-wide** is `static/css/{tokens,theme,layout,components,list-page,pages}/` and `static/js/{components,list-page,pages}/`. **Hub-owned** is `hubs/<hub>/static/<hub>/{css,js}/{components,pages}/` — the repeated app name is load-bearing (0022: without it a hub file sharing a portal file's name is silently shadowed), and a hub never defines a token or a theme.
