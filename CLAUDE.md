...
When building an HTML mockup/artifact to compare UI design alternatives, style it in the app's **Soft** theme (`data-theme="pastel"`), light mode — not the cool blue-grey default. Pull the actual token values from `static/css/theme/light.css` + the `[data-theme="pastel"]` block in `static/css/theme/themes.css` rather than approximating. Remember the editable-field-background rule (`DES-F1`, `static/css/theme/light.css`): pickable/label-like chrome (segmented options, fused-field labels) fills with `--bg-well`, actual text/search fields fill with `--bg-surface-alt` — never plain white `--bg-surface`, which is reserved for cards/read-only surfaces.
