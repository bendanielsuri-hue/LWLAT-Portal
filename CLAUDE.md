...
Default five-role vocabulary (needs-triage, needs-info, ready-for-agent, ready-for-human, wontfix), unchanged, plus two AFK blocker labels (`needs-stronger-model`, `needs-budget-approval`) that mark a fully-specified ticket as unsuitable for an unattended run and make `.github/workflows/afk-agent.yml` skip it, and `ready-for-review`, which that workflow swaps in once a run has opened a PR. Judge and apply both every time you apply `ready-for-agent`, and say in your reply which you applied and why — see `docs/agents/triage-labels.md`.
