...
- School scoping (the sidebar switcher's `all`/`primary`/`secondary`/`School.id` key) goes through `core.school_scope.SchoolScope` — `scope.narrow(qs, via=..., mat_wide=...)`, plus `selects_every_school` and `is_aggregate`, which look like the same question and are not. The four-branch cascade used to be written out five times across two apps. `core.identity`'s `staff_queryset_for_school_key`/`student_queryset_for_school_key`/`is_aggregate_school_key` still exist and are still the names to call; they are thin wrappers now. The selection itself is stored **once**, in the `current_school_key` cookie, and `core.identity.current_school_key` is the only reader — it absorbs a garbage or stale cookie into `'all'` so the sidebar label, the data and the settings can't disagree. Client-side, read it back with `static/js/layout/school-key.js`; never store a second copy (#196 — a localStorage copy of the display *name* did exactly that, and a rename desynchronised it permanently).
...
- **An icon file is a static asset referenced by `<svg><use href="{% static ... %}#icon">`, not a template that gets inlined** — two things follow, and both had already shipped broken. Its root `<svg id="icon">` carries a `viewBox` and **no `width`/`height`**: when `<use>` targets an `<svg>` element, that element's own attributes size the instance and the CSS on the host `<svg>` can no longer reach it, so every icon in the app renders at whatever the file says instead of what the call site asked for (this is what made every icon small at once when icons moved out of templates — CSS used to win because the markup was inlined). And the file is served raw, so Django template syntax in it is never processed: use an XML comment, never `{% comment %}`, which leaves literal text before the root element and makes the file unparseable. The call site owns the size, which means a call site that sets none gets the 300×150 default rather than its parent's box.
...
- Three advisory checkers, none of them commit hooks — `check_file_size.py` (code lines vs the ~600 trigger), `check_inline_js.py`, `check_stale_comments.py`. Crossing the size trigger is a question ("one module or two?"), and a legitimate "one" is answered in the file's header comment as `SIZE-ANSWERED: <code lines at the time> - <why>` — that's `(ENG-S3)`. The marker silences the checker only while the file is still the one that was answered about: **any** growth re-asks, as does a change of more than 10% in either direction without growth (a rewrite). A small shrink stays quiet, so tidying doesn't cost a re-answer. The finding then carries both numbers, and re-answering is editing the number. `check_file_size.py` covers the app's **Python** as well as its static assets (migrations and tests excluded, docstrings counted as comments the way a `.js` header comment is): the trigger is a property of how much code a reader has to hold, not of the language, and the file that proved it was `hubs/inclusion/panel/views.py` at 2,915 code lines with nothing that would ever have mentioned it (#220).
