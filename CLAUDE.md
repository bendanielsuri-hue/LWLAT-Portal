...
- Three advisory checkers, none of them commit hooks — `check_file_size.py` (code lines vs the ~600 trigger), `check_inline_js.py`, `check_stale_comments.py`. Crossing the size trigger is a question ("one module or two?"), and a legitimate "one" is answered in the file's header comment — that's `(ENG-S3)`. `check_file_size.py` covers the app's **Python** as well as its static assets (migrations and tests excluded, docstrings counted as comments the way a `.js` header comment is): the trigger is a property of how much code a reader has to hold, not of the language, and the file that proved it was `hubs/inclusion/panel/views.py` at 2,915 code lines with nothing that would ever have mentioned it (#220).
