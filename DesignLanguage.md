...
- One bordered box: `border: 1px solid var(--border-strong); border-radius: var(--radius-sm)`, options divided by `border-right: 1px solid var(--border-strong)` rather than a gap between separate boxes. Uses `--border-strong` (not the lighter `--border-color`) so the control reads clearly against `--bg-well`'s own fill — this was tried as a one-off override on Actions' row-detail status control (#12) first, then promoted to the shared component default since every segmented control benefits from the same contrast.
