...
- **U7.** Every row or card a user can click carries the same shared selection state as every other one — the pointer/focus marker on hover, and a persistent highlight on the one that was clicked. It is one package applied wholesale, not a per-page judgement about whether a particular list's selection "means" anything: a list that opts out looks broken next to its siblings, and the inconsistency is only ever noticed live. Reach for the shared implementation rather than restating its values, and keep the list container and its rows in step — the marker lives on the row, the persistent highlight is driven by the container, so a row styled without its container wired up gives hover but silently never highlights. Genuinely non-interactive rows (read-only history, a row that only exists to be dragged) are the only exclusion.
