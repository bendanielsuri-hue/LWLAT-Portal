...
- **U17.** Never bold text on hover/focus by changing its real font weight — a heavier weight is wider, so it can wrap a fitting single-line label onto an extra line, or nudge a sibling element (e.g. a pill) sideways, the moment the pointer or focus arrives. Fake the weight instead with a shadow effect that reads as heavier without changing the text's own metrics — see the shared token this resolves to for the current strength, arrived at by prototyping several side by side. A genuinely persistent state (selected/active, not hover/focus) can still use a real weight change since it doesn't flicker on and off.
