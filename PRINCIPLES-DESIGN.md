...
- **L10.** At the narrowest breakpoint a layout supports, drop a divider line separating repeating sections in favour of whitespace alone — screen space is too tight there for a rule to read as structure rather than clutter, and touch-sized layouts already lean on spacing/grouping over rules generally. Keep the line at every wider breakpoint where the same content still fits comfortably; whitespace-only is the narrowest-width exception, not the default to reach for everywhere.
